...
def train_model(train_loader, send_data_loader, file_name="./pth/vae.pth"): # VAE-GMMモデルを学習させる際に使用
    ...
    for i in range(5):
        print(f"相互学習{i+1}回目")
        vae_z = vae_module.train( gmm_mean=gmm_mu, epoch=50, first=flag, train_loader=train_loader, send_data_loader=send_data_loader, save_dir=file_name)
        gmm_pdz, gmm_mu = gmm_module.train( data=vae_z, K=10, num_itr=100) # gdm_pdz:(600,10), gmm_mu:(600,20)
        flag = False
    # 分類精度を測る
    vae_z = vae_module.send_latent(send_data_loader=send_data_loader,load_dir=file_name)
    gmm_pdz, gmm_mu, classes = gmm_module.test( data=vae_z, K=10, load_dir="model")
    vae_module.plot_latent(send_data_loader=send_data_loader, load_dir=file_name, image_name="latent_space", classes=classes)

def inference(send_data_loader, file_name="./pth/vae.pth"):
    # 分類精度を測る
    vae_z = vae_module.send_latent(send_data_loader=send_data_loader, load_dir=file_name)
    gmm_pdz, gmm_mu, classes = gmm_module.test( data=vae_z, K=10, load_dir="model")    
    vae_module.plot_latent(send_data_loader=send_data_loader, load_dir=file_name, image_name="a", classes=classes)
